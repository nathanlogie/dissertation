...
def adversarial_main(filepath, sensitive_attribute, target_column):
    ...
    technique = AdversarialDebiasing(privileged_groups=[{sensitive_attribute: 1}],
                                     unprivileged_groups=[{sensitive_attribute: 0}],
                                     scope_name='adv_debiasing',
                                     sess=sess, debias=True)
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        curr = adversarial_main(dataset[0], dataset[1], dataset[2])

        curr_df = pd.DataFrame([curr])
        curr_df["Dataset"] = dataset[3]
        curr_df["Run Type"] = "Adversarial Debiasing"

        total.append(curr_df)

    pd.concat(total).to_csv("adversarial_debiasing_results.csv", index=False)
