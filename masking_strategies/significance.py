...
df = pd.read_csv("masking_results.csv")
...
for model in df["Model"].unique():
    ...
    for dataset in df["Dataset"].unique():
        ...
        for metric in metrics:
            baseline = curr[curr["Masking Strategy"] == "Baseline"][metric]
            expanded = curr[curr["Masking Strategy"] == "Expanded"][metric]

            t_stat, p_value = ttest_ind(expanded, baseline, equal_var=False)
            ...
            toster, _, _ = tost_test(expanded, baseline)
            if toster:
                print(f"{metric}: {p_value} : {dataset} : {model}")
...
for metric, count in tost_counter.items():
    print(f"{metric}: {count}")