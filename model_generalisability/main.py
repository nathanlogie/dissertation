...
def main():
    ...
    models = [("Logistic Regression", LogisticRegression(solver='liblinear', random_state=1)),
    ...
    main_runs = [("Baseline", baseline_main), ("Adaptive Masking", adaptive_baseline_main)]
    ...
    combined_results_df = combined_results_df[
        ["Run Type", "Dataset", "Model"] + [col for col in combined_results_df.columns if
                                            col not in ["Run Type", "Dataset", "Model"]]]

    print(combined_results_df)
    combined_results_df.to_csv("model_generalisability.csv", index=False)

    bias_results = combined_results_df[[col for col in combined_results_df.columns if
        col not in ["Accuracy", "Bal. Acc.", "Precision", "Recall", "F1 Score"]]]
    bias_results.to_csv("bias_only_model_generalisability_results.csv", index=False)

    performance_results = combined_results_df[[col for col in combined_results_df.columns if
        col not in ["Disparate Impact", "Statistical Parity Difference", "Average Odds Difference",
